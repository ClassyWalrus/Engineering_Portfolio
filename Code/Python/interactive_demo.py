# Based on code from https://github.com/Jarvis73/Moving-Least-Squares
...
def demo(image, p, q, mode):
    ...
    pilim = Image.open(image)
    imagedpi = pilim.info['dpi']
    ...
    p = [[item[1],height-item[0]] for item in p]
    p = np.array(p, dtype=np.float64)
    q = [[item[1],height-item[0]] for item in q]
    q = np.array(q, dtype=np.float64)


    x = mlsd.mlsd_vec(X, p, q)
    plt.rcParams['lines.markersize'] = 0.5
    fig = plt.figure(figsize=(1.054*width/imagedpi[0],1.054*height/imagedpi[1]))  # factor of 1.054 is a fudge factor; can't find a better way.
    fig.dpi = imagedpi[0]
    ...
    plt.scatter(np.reshape(x[:,0],-1), np.reshape(x[:,1],-1), c=np.reshape(image,[-1,3])/255.0)
    flobj = tf.NamedTemporaryFile()
    #fig.savefig(flobj.name, dpi=imagedpi[0], format='png', bbox_inches="tight")
    fig.savefig(flobj.name, dpi=imagedpi[0], format='png')
    pilaug1 = Image.open(flobj.name)
    aug1 = np.array(pilaug1)
    del flobj
